...
- **静的 Embedding**：純粋なテーブル参照＋平均で、Transformer の順伝播が不要、CPU 上で約 0.05 ミリ秒/行；根本的な限界は文脈を認識しないことと、長文のプーリングによる意味の希薄化。軽量な畳み込みアダプタ（推論時に文脈に応じて token ベクトルを微調整）は、小型の稠密モデル（MiniLM-L6）の精度の約 94% を約 100 倍の速度で達成できる——レイテンシに敏感な CPU/エッジシナリオの具体的な妥協点だが、より複雑な手法（より強力な教師モデル、直接対比学習）はさらなる突破にはつながらない。
...
LlamaIndex が静的 Embedding + MaxSim を実験（minishlab/potion-base-32M）：軽量な畳み込みアダプタ（530KB）が MiniLM-L6 の精度の約 94%（0.526 対 0.562 NDCG）を約 100 倍の速度で達成；より強力な教師モデルへの切り替え、直接対比学習、embedding テーブル全体のファインチューニングはいずれもさらなる突破につながらなかった——静的 Embedding の精度上限に関する堅実な実証的探索（詳細は [rag](/ja/topics/rag)）。
