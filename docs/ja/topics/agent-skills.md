...
| ponytail | 新登場（2026-09-03 Trending 入り、9 日間で約 12.2 万スターに到達、直近で最も増加速度が速い） | 強制的な「意思決定のはしご」で AI 生成コード量を圧縮、公式ベンチマークで平均 54% のコード出力削減 |
...
ponytail が登場（GitHub Trending、9 日間で約 12.2 万スターに到達、直近の skill リポジトリの中で最も増加速度が速い）：Claude Code/Codex/Copilot CLI など 15+ プラットフォームに対応する汎用 skill で、核心はコードを書く前の強制的な「意思決定のはしご」——本当に新しいコードが必要か、コードベース/標準ライブラリ/プラットフォームのネイティブ機能/既存の依存関係でできないか、を段階的に自問し、すべて「否」の場合にのみ着手する；公式ベンチマークでは平均でコード出力を 54% 削減、最大で 94% 削減。「Agent の手抜き・過剰実装を防ぐ」という流れの継続（以前 Addy Osmani の agent-skills が「手抜きの言い訳への反論」を提唱していた）であり、「コードを書かない」こと自体を提唱ではなく強制フローにした、この流れのさらなる先鋭化。「制約系」skill の市場需要が「能力拡張系」に劣らない可能性も裏付ける。
