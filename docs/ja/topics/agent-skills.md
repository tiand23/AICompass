...
- **供給側の階層**：標準を定める公式ベンダー（Anthropic）→ ハイパースケーラー公式の製品ライン合集（google/skills、自社全製品をカバーする「ドキュメントの実行可能版」）→ 著名な独立エンジニアの個人経験萃取（Addy Osmani、Matt Pocock）→ 匿名/コミュニティリポジトリ——階層ごとに信頼性と更新の同期性が異なり、選定時は出典の階層で評価する。
