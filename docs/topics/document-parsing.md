...
- **"流畅型"错误比"乱码型"错误更危险**：传统 OCR 出错产出肉眼可辨的乱码，LLM OCR 出错产出流畅、看似合理但内容有误的文本——视觉证据模糊时模型偏向"语言上最可能"而非"图像最支持"的答案，账号/编号等高熵字符串恰恰是语言先验最弱、最易被幻觉污染之处。
- **整页平均指标会掩盖关键字段错误**：字符/词错误率在整页取平均，一页 99% 准确率若错的 1% 是总金额或编号字段，这页数据依然无用——验收应按字段类型（尤其高熵字段）分别测准确率，而非只看整页平均分。
- **持久化工作流作为大规模解析管线的调度底座**：文档处理天然是长任务、多阶段、易失败的场景（单份文档可能需要 OCR+视觉模型+多轮处理）——用 Temporal 一类持久化工作流引擎把"资源协调"本身建模成可观测的工作流实体（如信号量 Workflow 做并发控制），比手写锁/重试计数器更可靠，失败任务能从检查点自动续跑而非从头重来（详见 [agent-workflow](/topics/agent-workflow)）。
...
LlamaIndex 发文指出 LLM OCR 的错误不是变少了而是变得更难被发现：拆解三种主流架构（OCR+LLM 纠错、VLM 原生转录、agentic 分割路由）各自的失败模式，核心机制是视觉证据模糊时模型偏向语言先验而非图像证据；并指出 CER/WER 类整页平均指标会掩盖关键字段错误——生产系统需要分割、路由、校验回路、像素级引用等架构级控制手段。
