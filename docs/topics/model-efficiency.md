...
- **量化感知修复（QAH）**：与从"已退化的压缩后检查点"当教师的常规 QAT 不同，QAH 直接从压缩前的原始满血模型蒸馏（冻结教师 logits + KL 散度损失，教师输出分布与架构无关）——避免学生被锚定在退化教师的天花板之下；实测 4-bit 压缩模型在多数基准上反超自己的 16-bit 源模型，且训练仅需约 100 步（QAT 约 700 步后开始崩溃）。
...
Multiverse Computing 发布量化感知修复（QAH）：4-bit 压缩模型在 GPT-OSS 120B→60B 实测中 9 个基准里 7 个反超自己的 16-bit 源模型，权重内存降至约四分之一——与 08-19 QAD、08-20 DSpark 同属"端侧模型提速不掉质量"效率簇的第三个解法。同日 IBM 发布 Granite 4.2：Apache 2.0 开源推理模型家族（3B/8B/30B），Agentic RL 阶段在真实沙箱环境（而非模拟）训练 SWE Agent，30B 版本 SWE-Bench Verified 达 57%（详见 [deep-agents](/topics/deep-agents)）。
