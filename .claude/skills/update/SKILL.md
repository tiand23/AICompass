...
`docs/map.md` 按**生成式 AI 开发栈的生态位**组织，固定骨架（不要增删一级格子，子格子可按需细分）：

1. 模型层（基础模型 / 多模态生成 / 效率与小型化：SLM・蒸馏・量化）
2. 知识与检索（知识库、RAG、向量库、KG）
3. Agent 开发（编排与 Workflow / 运行时与云：托管运行时・平台・Foundry / 协议与互操作：MCP・A2A・AG-UI）
4. 推理与部署
5. 评估
6. 安全
7. 编码工具

规则：每个 Topic **只住一个格子**（相关格子用"参见"链接）；空格子保留并标"待积累"——它表示在跟踪的生态位，不是错误。每次运行后：

1. 新建的 Topic 放进对应格子（一行：Topic 链接｜是什么｜解决什么问题｜状态｜最近动态日期）；如果它填的是空格子，把"待积累"段落换成表格。
